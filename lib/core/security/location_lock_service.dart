...
class LocationLockService {
  ...
  static Future<bool> isOutsideTrustedLocation() async {
    ...
    if (!isTrustedLocationConfigured()) {
      debugPrint('  Trusted location NOT configured - allowing access');
      return false;
    }

    final isServiceEnabled = await Geolocator.isLocationServiceEnabled();
    if (!isServiceEnabled) {
      debugPrint('  Location service DISABLED on device - blocking access');
      return true;
    }
    ...
    if (permission == LocationPermission.denied ||
      ...
      debugPrint('  Location permission DENIED - blocking access');
      return true;
    }
